...
class ChargebackPredictor:
    ...
    def train(self, X: np.ndarray, y_fraud: np.ndarray) -> None:
        ...
        cb_legit = rng.choice(
            legit_idx, size=max(1, int(len(legit_idx) * 0.002)), replace=False
        )
        y_chargeback[cb_legit] = 1

        logger.info(
            f"  Chargeback labels: {y_chargeback.sum()} positive / {len(y_chargeback)} total"
        )
        ...
        try:
            ...
            logger.warning(
                "imblearn not installed — training without SMOTE. Run: pip install imbalanced-learn"
            )
...
def get_chargeback_predictor() -> ChargebackPredictor:
    ...
    if _predictor is None:
        ...
        try:
            ...
            logger.info(
                "Chargeback model not found — run train_payment_models.py to generate it."
            )
    return _predictor
