...
FEW_SHOT_EXAMPLES = [
    ...
    },
    {
        "features": [
            ...
            {
                "feature": "device_seen_before",
                "impact": 0.02,
                "direction": "suppresses_fraud",
            },
    ...
    },
    {
        "features": [
            {
                "feature": "velocity_24h_amount_log",
                "impact": 1.7,
                "direction": "elevates_fraud",
            },
            {"feature": "bin_risk_score", "impact": 1.0, "direction": "elevates_fraud"},
...
def _mock_explain(
    ...
    if decision == "DECLINE":
        ...
        if suppressing:
            parts.append(
                f"Mitigating factors ({', '.join(label(f) for f in suppressing)}) were outweighed."
            )
        ...
        if uncertainty_type == "EVIDENCE_CONFLICT":
            ...
            parts.append(
                "ensemble models disagree — epistemic uncertainty too high for automation."
            )
        else:
        ...
        if suppressing:
            parts.append(
                f"Mitigating signals: {', '.join(label(f) for f in suppressing)}."
            )
    ...
    async def explain(
        ...
        if self.redis:
            try:
                ...
                if cached:
                    logger.debug(
                        f"Explanation cache HIT ({(time.perf_counter() - t0) * 1000:.1f}ms)"
                    )
                    return cached
            except Exception:
                pass
        ...
        explanation = await self._generate(
            decision, shap_features, amount, merchant_id, uncertainty_type
        )
        ...
        logger.info(
            f"Explanation generated ({self.provider}, {ms:.1f}ms): {explanation[:60]}..."
        )
        return explanation

    async def _generate(
        self, decision, shap_features, amount, merchant_id, uncertainty_type
    ) -> str:
