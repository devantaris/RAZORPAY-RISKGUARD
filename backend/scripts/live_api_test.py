    ...
    },
    {
        ...
        "payload": {
            "transactions": [
                ...
                {
                    "transaction_id": "txn_B_002",
                    "merchant_id": "merch_food_002",
                    "amount": 280.0,
                    "currency": "INR",
                    "payment_method": "UPI",
                },
                {
                    "transaction_id": "txn_B_003",
                    "merchant_id": "merch_petrol_001",
                    "amount": 2100.0,
                    "currency": "INR",
                    "payment_method": "CARD",
                },
...
for s in scenarios:
    ...
    if is_batch:
        print(
            f"  Batch results ({resp.get('total_ms', 0):.1f}ms | {resp.get('throughput_tps', 0):.0f} TPS):"
        )
        for r in resp.get("results", []):
            print(
                f"    txn={r['transaction_id']}  decision={r['decision']:<8}  conf={r['confidence']:.3f}  stage={r['stage_reached']}"
            )
    else:
        ...
        print(
            f"  Latency      : {resp.get('inference_ms', ms):.1f}ms  (wall: {ms:.1f}ms)"
        )
        ...
        if rr.get("shap_top_features"):
            ...
            for f in rr["shap_top_features"]:
                print(
                    f"    {f['feature']:<30} impact={f['impact']:.4f}  [{f['direction']}]"
                )
...
h = (
    post("/v1/health", None)
    if False
    else json.loads(req.urlopen(f"{BASE}/v1/health", timeout=5).read())
)
