...
def build_features(df):
    ...
    X["bin_risk_score"]          = df["card_bin"].apply(lambda b: 1.0 if str(b) in HIGH_RISK_BINS else 0.0)
...
v1 = V1Ensemble(); v1.load(ARTIFACT_DIR)
v2 = V2SVM();      v2.load(ARTIFACT_DIR)
v3_svm = V3SVM();  v3_svm.load(ARTIFACT_DIR)
v4 = V4ShapDeferral(); v4.load(ARTIFACT_DIR)
...
for state in ["APPROVE","DECLINE","STEP_UP","PEND"]:
    mask = final == state
    n, nf, nl = mask.sum(), int((mask & (y_test==1)).sum()), int((mask & (y_test==0)).sum())
...
print()
print("  CORE METRICS")
print(f"  DECLINE Precision : {('100% [PASS]' if false_blocks==0 else str(round(true_declines/(true_declines+false_blocks+1e-9)*100,1))+'% [FAIL]')}")
print(f"  False Blocks      : {false_blocks}  (must be 0)")
print(f"  Recall (DECLINE)  : {true_declines}/{total_fraud} = {true_declines/max(total_fraud,1)*100:.1f}%")
print(f"  Recall (any flag) : {flagged}/{total_fraud} = {flagged/max(total_fraud,1)*100:.1f}%")
