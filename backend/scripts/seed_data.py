...
MERCHANT_IDS = [
    f"merch_{cat}_{i:03d}" for cat, _ in MERCHANT_CATEGORIES.items() for i in range(10)
]
...
def pick_card_bin(is_fraud: bool) -> tuple[str, str]:
    if is_fraud and random.random() < 0.55:
        ...
        network = random.choice(
            ["visa_in", "mastercard", "rupay", "visa_in", "mastercard"]
        )
...
def generate_time_of_day(is_fraud: bool) -> tuple[datetime, bool]:
    ...
    ts = now - timedelta(
        days=days_back, hours=(now.hour - hour) % 24, minutes=random.randint(0, 59)
    )
...
def generate_transactions(n: int = N_TRANSACTIONS) -> pd.DataFrame:
    ...
    for i, label in enumerate(labels):
        ...
        merchant_id = random.choice(
            [m for m in MERCHANT_IDS if category in m] or MERCHANT_IDS
        )
        ...
        upi_vpa = (
            f"{fake.user_name()}{random.choice(UPI_SUFFIXES)}"
        ...
        device_seen_before = (
            (random.random() > 0.70) if is_fraud else (random.random() > 0.20)
        )
